...
export default function UpdateProductPage() { // Renamed component
  ...
  useEffect(() => {
    if (productId && categories.length > 0) { // Ensure categories are loaded before trying to match categoryId
      const fetchProductData = async () => {
        setIsLoadingData(true);
        try {
          const response = await axios.get<ProductData>(`/api/protected/products/${productId}`);
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const allowedNewUploads = 5 - (existingImages.length + selectedFiles.length);

      if (filesArray.length > allowedNewUploads) {
        toast.error(`شما فقط می‌توانید ${allowedNewUploads} تصویر دیگر اضافه کنید.`);
  ...
  const handleSlugGeneration = () => {
    ...
    if (formData.name) { // Simpler check for update page
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (removedImageKeys.length > 0) {
      submissionFormData.append("removedS3Keys", JSON.stringify(removedImageKeys));
    }
    ...
    <AuthCheck allowedRoles={["SUPERADMIN", "PRODUCTMANAGER"]}>
      <div>
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">ویرایش محصول</h1> {/* Updated heading */}
        ...
        {isLoadingData || (loadingCategories && categories.length === 0 && !formData.categoryId) ? ( // Combined loading state
          ...
          <div className="bg-white rounded-lg shadow p-6 text-center">
            ...
            <p className="mt-2">
              برای ویرایش یا ایجاد محصول، ابتدا باید حداقل یک دسته‌بندی موجود باشد.
            </p>
            <div className="mt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {existingImages.length > 0 && (
                  <div className="col-span-1 md:col-span-2 mt-4">
                    ...
                    {existingImages.length === 0 && !isLoadingData && (
                       <p className="text-sm text-gray-500">این محصول در حال حاضر تصویری ندارد.</p>
                    )}
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      {existingImages.map((image) => (
                        <div key={image.id || image.s3Key} className="relative group">
                          <img
                            src={image.url}
                          ...
                          <button
                            type="button"
                            onClick={() => handleRemoveExistingImage(image.s3Key)}
                ...
                <div className="col-span-1 md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {existingImages.length > 0 ? "افزودن تصاویر جدید" : "بارگذاری تصاویر محصول"} (حداکثر مجموعاً {maxTotalImages} تصویر)
                  </label>
                  ...
                  {!canUploadMore && (
                    <p className="mt-2 text-sm text-yellow-600">
                      به حداکثر تعداد تصاویر (۵ تصویر) رسیده‌اید. برای افزودن تصویر جدید، ابتدا یک تصویر موجود را حذف کنید.
                    </p>
                  )}
                </div>
              ...
              <div className="mt-8 flex justify-end space-x-4 space-x-reverse">
                <button
                  ...
                  className={`px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition ${
                    isSubmitting || isLoadingData ? "opacity-70 cursor-not-allowed" : ""
                  }`}
                >
                  {isSubmitting ? (
